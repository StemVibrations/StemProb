...
def run_2d_model():
    ...
    # Make an array from normal distribution with mean -1000 and std 200 with NUM_SIMULATIONS number of elements based on the SEEDS
    rng = np.random.default_rng(SEEDS)
    VerticalLoad = rng.normal(loc=-1000, scale=200, size=NUM_SIMULATIONS)
    ...
    for i in range(NUM_SIMULATIONS):
        ...
        point_load = PointLoad(active=[False, True, False], value=[0, -1000, 0])
        moving_load = MovingLoad(load=[0.0, VerticalLoad[i], 0.0],
                                 direction=[1, 0, 0],
                                 velocity=0.07,
                                 origin=[0.0, 1.0, 0.0],
                                 offset=0.0)
        ...
        pass
        #model.synchronise_geometry()
        # model.generate_mesh()                                            # why it does not work???
        ...
        with open(path_to_results) as f:
            ...
            results_node = calculated_response['NODE_5']     ###Why the NODE_5 is hardcoded here? 
    ...
    import matplotlib.pyplot as plt
    fig, ax = plt.subplots()
    #for each i print the Vertical load value close to its plot on the following graph
    for i in range(NUM_SIMULATIONS):
        ax.plot(responses_var['TIME'], responses_var['DISPLACEMENT_X'][i], color='darkgray', alpha=0.3)
        #plot the corresponding value of Vertical load[Num_simulation] close to the graph

        # Find the last point of the curve to annotate
    ...
    for i in range(NUM_SIMULATIONS):
        ax.plot(responses_var['TIME'], responses_var['DISPLACEMENT_Y'][i], color='red', alpha=0.3)
